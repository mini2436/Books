...
if (-not $SkipBuild) {
    ...
    $javaVersionOutput = if ($javaCommand) { (& java -version 2>&1 | Out-String) } else { '' }
...
try {
    ...
    $dockerArguments = @(
        'compose', 'run', '--rm',
        '-e', 'APP_BACKFILL_BOOK_RESOURCES=true'
    )
    if ($Overwrite) {
        $dockerArguments += @('-e', 'APP_BACKFILL_BOOK_RESOURCES_OVERWRITE=true')
    }
    $dockerArguments += 'backend'

    & docker @dockerArguments
    if ($LASTEXITCODE -ne 0) {
        throw "Book resource backfill failed with exit code $LASTEXITCODE"
    }
} finally {
    Pop-Location
}
